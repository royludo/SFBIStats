...
def run(job_list, output_dir):
    ...
    df_level_region = gb_region.contract_subtype.aggregate({'Master': count_master, 'PhD': count_phd})
    df_level_region['Total'] = df_level_region['Master'] + df_level_region['PhD']
    df_level_region['Master'] = df_level_region['Master']/df_level_region['Total']
    df_level_region['PhD'] = df_level_region['PhD']/df_level_region['Total']

    df_level_region = df_level_region.sort_values(by='Total')
    fig, ax = plt.subplots()
    ax = df_level_region.loc[:, ['Master', 'PhD']].plot(ax=ax, kind='barh', stacked=True, color=colors)
    ax.set_xlim([0,1])
    ax.set_ylabel('')
    plt.setp(ax.get_yticklines(), visible=False)
    i = 0
    ...
    fig.savefig(os.path.join(output_dir, 'summary_lins_7.svg'), bbox_inches='tight')
    plt.close(fig); del(fig)
    ...
    gb_region = df.groupby('region')
    df_type_region = gb_region.contract_type.value_counts(sort=True, ascending=True).unstack(level=-1).fillna(0)
    df_type_region['Total'] = df_type_region.sum(axis='columns')
    df_perc_region = df_type_region.div(df_type_region['Total'], axis='index')
    df_perc_region['Total'] = df_type_region['Total']

    df_perc_region = df_perc_region.sort_values(by='Total')
    fig, ax = plt.subplots()
    ax = df_perc_region.loc[:, ['CDD', 'CDI', 'Stage', u'Thèse']].plot(ax=ax, kind='barh', stacked=True, color=colors)
    ax.set_xlim([0,1])
    ax.set_ylabel('')
    plt.setp(ax.get_yticklines(), visible=False)
    i = 0
    ...
    fig.savefig(os.path.join(output_dir, 'summary_lins_8.svg'), bbox_inches='tight')
    plt.close(fig); del(fig)
